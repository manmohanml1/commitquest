...
Identity gates cover relative return-path validation, one-time and expiring OAuth state, PKCE, zero requested scope, explicit `prompt=select_account` propagation, unexpected-scope rejection, stable GitHub numeric identity reconciliation, session rotation and expiry, HMAC-only persistence, Secure/HttpOnly/SameSite cookies, double-submit CSRF, Origin/Referer checks, logout, and cascading account deletion. Angular behavior distinguishes normal same-account reconnect, user-requested GitHub account choice, post-deletion account choice, Escape-dismissable modal behavior, free-host vault awakening, and an OAuth preflight that prevents provider navigation until the identity host returns an authenticated or expected unauthenticated response. The v0.4 component-style warning budget is capped at 16 KB with a 20 KB hard failure; raising either limit requires explicit review.
...
The first routed-shell slice covers owner-session loading, signed-out and unavailable-state separation, safe campaign selection, OAuth return-path preservation, sign-out cleanup, direct `/app` route refresh, responsive navigation without horizontal overflow, route titles, console errors, and continued rendering of the public route. Overview, Vault, and Account are migration destinations; the v0.4 lifecycle and destructive-action suites remain binding until their focused v0.5 owners replace them.

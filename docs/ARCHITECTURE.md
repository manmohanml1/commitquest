...
V0.5 introduces a routed authenticated shell while preserving the independently renderable public experience. Session, campaign library, selected campaign, connection status, and design mode are shell-level state. Route components depend on application services rather than generated HTTP clients, and Modern/Chronicle variants share one semantic component tree. ADR 0008 fixes the GitHub App permission and token boundary before provider registration.
